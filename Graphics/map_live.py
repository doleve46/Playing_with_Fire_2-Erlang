...
class CompleteGameVisualizer:
    ...
    def parse_erlang_map_file(self, filename):
        """Parse Erlang map file and extract complete game state"""
        try:
            with open(filename, 'r') as file:
                content = file.read()

            # Find the get_map() function or get_game_state() function
            for function_name in ["get_game_state()", "get_map()"]:
                start_marker = function_name + " ->"
                start_pos = content.find(start_marker)
                if start_pos != -1:
                    break
            else:
                raise ValueError("Could not find get_game_state() or get_map() function in file")

            # Find the opening bracket of the data structure
            list_start = content.find("[", start_pos)
            if list_start == -1:
                list_start = content.find("{", start_pos)
            if list_start == -1:
                raise ValueError("Could not find opening bracket/brace of data")
            ...
            data_str = content[list_start:end_pos + 1]
            print(f"🔍 Extracted Erlang data: {data_str[:200]}...")

            # Parse the Erlang data
            result = self.parse_erlang_data_structure(data_str)
            print(f"✅ Successfully parsed game state from Erlang file")
            return result
    ...
    def parse_erlang_data_structure(self, data_str):
        """Parse Erlang data structure (list or tuple format)"""
        try:
            ...
            # Handle tuple format
                ...
                raise ValueError("Unknown data structure format")

        except Exception as e:
            print(f"⚠️ Error parsing Erlang data structure: {e}")
            return self.get_fallback_game_state()
    ...
    def parse_erlang_tuple(self, tuple_str):
        """Parse Erlang tuple syntax - supports both 4-tuple and 6-tuple formats"""
        try:
            # Remove outer braces
            ...
            # Support both 4-tuple (basic) and 6-tuple (enhanced) formats
            if len(parts) >= 4:
                # Pad with 'none' if we have less than 6 elements
                while len(parts) < 6:
                    parts.append('none')
                return tuple(parts[:6])  # Take first 6 elements
            else:
                print(f"⚠️ Tuple has {len(parts)} elements, expected at least 4: {parts}")
                return None
    ...
    def get_fallback_game_state(self):
        """Fallback game state if file parsing fails"""
        return self.get_fallback_grid()

    def get_fallback_grid(self):
        """Fallback grid data if file parsing fails"""
        # Create a simple test grid with proper 6-tuple structure
        grid = []

        for x in range(MAP_SIZE):
            row = []
            for y in range(MAP_SIZE):
                # Format: (tile_type, powerup_type, bomb_info, player_info, explosion_info, special_info)

                # Border walls
                if x == 0 or x == MAP_SIZE - 1 or y == 0 or y == MAP_SIZE - 1:
                    row.append(('unbreakable', 'none', 'none', 'none', 'none', 'none'))
                # Player start positions
                elif (x, y) == (1, 1):
                    row.append(('player_start', 'none', 'none', 'player_1', 'none', 'none'))
                elif (x, y) == (1, MAP_SIZE - 2):
                    row.append(('player_start', 'none', 'none', 'player_2', 'none', 'none'))
                elif (x, y) == (MAP_SIZE - 2, 1):
                    row.append(('player_start', 'none', 'none', 'player_3', 'none', 'none'))
                elif (x, y) == (MAP_SIZE - 2, MAP_SIZE - 2):
                    row.append(('player_start', 'none', 'none', 'player_4', 'none', 'none'))
                # Some random breakable walls with power-ups
                elif random.random() < 0.2 and x > 2 and y > 2 and x < MAP_SIZE - 3 and y < MAP_SIZE - 3:
                    powerups = ['none', 'none', 'none', 'move_speed', 'plus_bombs', 'bigger_explosion', 'kick_bomb']
                    powerup = random.choice(powerups)
                    row.append(('breakable', powerup, 'none', 'none', 'none', 'none'))
                # Free spaces
                else:
                    row.append(('free', 'none', 'none', 'none', 'none', 'none'))
            grid.append(row)
    ...
    def reload_game_state(self):
        """Reload complete game state and detect changes for animations"""
        # Store previous state
        self.previous_game_state = self.current_game_state.copy() if self.current_game_state else None

        # Load new state
        new_game_state = self.load_complete_game_state()

        if self.previous_game_state and new_game_state:
        ...
        print("✅ Game state reloaded successfully!")
    ...
    def run(self):
        ...
        while running:
            ...
            self.draw_enhanced_map()  # Map with live players, bombs, explosions
            self.draw_player_stats_panel()  # Live player stats and status
            self.draw_powerups_panel()  # Game status and power-ups reference
